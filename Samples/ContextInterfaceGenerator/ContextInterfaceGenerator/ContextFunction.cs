...
namespace ContextInterfaceGenerator
{
    public class ContextFunction
    {
        ...
        public ContextFunction(XmlNode node) : this()
        {
        ...
        {
            var list = Parameters.Select(prm => string.Format("{0}{1} {2}", prm.ByRef ? "ref " : string.Empty, prm.Type == "System.String" ? "string" : prm.Type + "?", prm.Name));
